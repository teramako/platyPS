...
Describe "Compare-CommandHelp can find differences" {
    ...
    It "Should properly identify the number of differences" {
        $result1.where({$_ -match "are not the same|are different"}).Count | Should -Be 8
    }

    It "Should properly identify the elements which are different" {
        $expected = "CommandHelp.Locale",
            "CommandHelp.Syntax.ParameterSetName", "CommandHelp.Syntax.ParameterSetName", "CommandHelp.Syntax.ParameterSetName", "CommandHelp.Syntax.ParameterSetName",
